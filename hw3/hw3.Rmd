...
For this assignment, we will use the [Fisher's Exact Test](http://en.wikipedia.org/wiki/Fisher%27s_exact_test) and the [Chi-square test](http://en.wikipedia.org/wiki/Chi-square_test).  We denote the odds ratio as $\theta$. There are six problems. See the document in the repository for more details. 
...
substrate=array(c(15, 20, 14, 6, 8, 5, 7, 1),
                dim = c(4, 2),
                dimnames = list(
                    distance = c("Vegetation", "Shoreline", "Water", "Structures"),
                    bird = c("Heron", "Egret")))
